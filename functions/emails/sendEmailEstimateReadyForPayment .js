...
const sendEmailEstimateReadyForPayment = functions.firestore
  .document("reservations/{reservationId}/devis/{devisId}")
  .onCreate(async (snap, context) => {
    // Fetch the reservation data to get client details
    const reservationDoc = await snap.ref.parent.parent.get();
    const reservationData = reservationDoc.data();
    const clientEmail = reservationData.email;
    const clientName = reservationData.firstName;
...
<head>
  <title> Notification de Nouveau Devis </title>
  <!--[if !mso]><!-->
...
<body style="word-spacing:normal;background-color:#ffffff;">
  <div style="background-color:#ffffff;">
              ...
              <div class="mj-column-per-100 mj-outlook-group-fix" style="font-size:0px;text-align:left;direction:ltr;display:inline-block;vertical-align:top;width:100%;">
                <table border="0" cellpadding="0" cellspacing="0" role="presentation" style="vertical-align:top;" width="100%">
                  <tbody>
                    <tr>
                      <td align="center" style="font-size:0px;padding:10px 25px;word-break:break-word;">
                        <div style="font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:24px;font-weight:bold;line-height:1;text-align:center;color:#333333;">Un nouveau devis est disponible</div>
                      </td>
                    </tr>
                    <tr>
                      <td align="left" style="font-size:0px;padding:10px 25px;word-break:break-word;">
                        <div style="font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:16px;line-height:1;text-align:left;color:#666666;">Bonjour ${clientName},<br><br> Nous vous informons que la visite de votre propriété est terminée. Un nouveau devis est désormais disponible dans votre espace client. Veuillez vous connecter pour le consulter et procéder au paiement si vous le souhaitez.</div>
                      </td>
                    </tr>
                    <tr>
                        ...
                        <table border="0" cellpadding="0" cellspacing="0" role="presentation" style="border-collapse:separate;line-height:100%;">
                          <tr>
                            <td align="center" bgcolor="#007bff" role="presentation" style="border:none;border-radius:5px;cursor:auto;mso-padding-alt:10px 25px;background:#007bff;" valign="middle">
                              <a href="http://localhost:5173/" style="display:inline-block;background:#007bff;color:#ffffff;font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:13px;font-weight:normal;line-height:120%;margin:0;text-decoration:none;text-transform:none;padding:10px 25px;mso-padding-alt:0px;border-radius:5px;" target="_blank"> Voir le devis </a>
                            </td>
                          </tr>
                        </table>
                      </td>
                    ...
                    <tr>
                      <td align="left" style="font-size:0px;padding:10px 25px;word-break:break-word;">
                        <div style="font-family:Ubuntu, Helvetica, Arial, sans-serif;font-size:16px;line-height:1;text-align:left;color:#666666;">Merci de votre confiance,<br> L'équipe Sahel</div>
                      </td>
                    </tr>
                  </tbody>
                </table>
    ...
    try {
      console.log(
        `Sending estimate ready for payment email to: ${clientEmail}`
      );
      await apiInstance.sendTransacEmail(sendSmtpEmail);
      console.log("Estimate ready for payment email sent successfully");
    } catch (error) {
      console.error("Failed to send estimate ready for payment email:", error);
    }
  });

module.exports = { sendEmailEstimateReadyForPayment };
